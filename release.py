...
VERSION_PY = 'jupyterlab_proxy_gui/_version.py'
...
def tag(version, dryrun=False, kind=None):
    ...
    if dryrun:
        ...
        subprocess.run(['git', 'push', 'upstream', tag], shell=True)
...
def pypi(wheel=True, test=False):
    ...
    if test:
        ...
        subprocess.run(['twine', 'upload', '--repository-url', 'https://test.pypi.org/legacy/', 'dist/*'], shell=True)
    else:
...
def npmjs(dryrun=False):
    ...
    if dryrun:
        ...
        subprocess.run(['npm', 'publish', '--access', 'public', '--dry-run'], shell=True)
    else:
        # build and release
        subprocess.run(['npm', 'publish', '--access', 'public'], shell=True)
...
def labExtensionVersion(dryrun=False, version=None):
    if version:
        ...
        if dryrun:
            ...
            subprocess.run(force_ver_cmd, shell=True)
    else:
